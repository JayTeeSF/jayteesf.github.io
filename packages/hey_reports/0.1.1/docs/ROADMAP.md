...
Package code rewritten to dodge a compiler gap. Each is equivalent to
the natural form; revert it when the compiler lands the fix, and let
`bin/check`'s two-lane diff confirm.

- **`for x in call()` is written `let xs = call()` then `for x in xs`**
  (0.1.1; `src/svg.hey` `ReportsSvg.render_result`,
  `specs/main_spec.hey`). The compiled lane's MIR-to-LLVM emitter
  cannot lower `bytes()` (`unsupported boxed expression ... bytes('' +
  value, 'utf8')`, from `ReportsText.char_count`), and that ONE line
  moves the whole program onto the legacy direct-MIR consumer. The
  legacy consumer then refuses to iterate a call result directly:
  `llvm backend supported subset cannot lower: for b353 element in call
  @t8.126 ReportsSvg.chrome(doc, theme, plot)`. `main_spec` did not
  build. Binding the call to a local first changes nothing else.
  Binding the `bytes()` argument to a local does NOT help; the emitter
  rejects `bytes()` itself, so the program still builds on the legacy
  consumer. Two compiler gaps, reported upstream: `bytes()` in the
  MIR-to-LLVM emitter, and for-over-call in the legacy consumer.
