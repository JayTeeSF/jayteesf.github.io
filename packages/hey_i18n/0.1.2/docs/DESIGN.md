...
- Hey stdlib only (`stdlib:Text`); no runtime package dependencies.
...
## 0.1.1 — the compiled lane, learned the hard way

0.1.0 shipped with interpreter-only gates and could not be compiled by any
consumer. Three classes of defect, all fixed here and all now guarded by
`tools/compiled_receipt.hey` (bin/check builds a real binary and demands
byte-identical output with the interpreter):

1. `Text.index_of` / `Text.slice` do not lower in the LLVM subset (they
   dispatch as `Collections.index_of`); the bare builtins `index_of` /
   `slice` do. `Text.trim` / `Text.lower` / `Text.replace_all` lower fine —
   "some Text.* is compiled-safe" is a trap; the receipt is the arbiter.
2. A string-or-nil-returning function either refuses to lower (small
   program, loud) or MISLOWERS into always-nil inside a larger build with
   no error at all (observed during this fix). Therefore nil never crosses
   this package's compiled API: `I18n.carries(catalog, chain, key)` is the
   boolean membership test, `t`/`format`/`plural`/`render_body` always
   return strings, and `lookup`/`plural_lookup` keep their nil contract but
   are documented interpreter-lane-only (specs and tooling).
3. The module moved from `src/I18n.hey` to the package root (`i18n.hey`):
   the toolchain resolves `pkg:<name>@<ver>/<module>` by path concatenation
   and never reads the manifest's modules map, so only root-level module
   files are importable by consumers today.

Also renamed on the way: no public module function carries a trailing `?`
(`carries`, not `carries?`) — a qualified call to one is a known LLVM
crash shape in consumer apps.
