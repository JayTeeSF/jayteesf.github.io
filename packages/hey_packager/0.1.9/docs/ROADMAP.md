...
## 0.1.9 — rewrites change the version and nothing else

- `bin/bump` keeps the final newline of VERSION, hey-package.json and every declared copy exactly as it was, including a missing one.
- `bin/update_packages` keeps the manifest's final newline and line endings when it moves a pin, and rewrites import references in place so each file keeps its mode and bytes.
- `update-packages` run for real against the registry with the real `hey` on hey_carplay: range dependencies re-resolved in the lock, one `pkg:` import moved, `hey package verify` and the full check passed.
