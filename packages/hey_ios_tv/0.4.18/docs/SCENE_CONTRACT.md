...
there is no OpenSSL for tvOS, so the Hey runtime compiles its TLS client out for
this target entirely. An HTTPS request issued from Hey on tvOS now returns a
named `tls_client_unavailable` error the program can branch on. Before 0.4.16 it
`abort()`ed against a bank of fake OpenSSL symbols; it has never worked, and it
must never silently fall back to plaintext.
...
## The settings menu (`allow_settings`, 0.4.13; the Screen row 0.4.14)

A television can be told to stop making noise — and, since 0.4.14, what
brightness to render at — from the remote in the room it is in. **Up** opens a
renderer-drawn overlay with three rows, navigated with Up/Down, changed with
Select, closed with Menu or Play/Pause:

| row | states | who acts on it |
|---|---|---|
| **Soundtrack** | On / Off | the renderer (`sound.file` and the attract reel) |
| **Sound effects** | On / Off | the renderer (the synthesised `sound.cue` palette) |
| **Screen** | Auto / Dark / Light | **the SERVER** — see below |

**The Screen row is the one setting the server acts on**, and it CYCLES rather
than toggles, because three states need every one of them reachable from a
single button. It is sent on every scene poll as a request header:

```
X-Hey-TV-Screen: auto | dark | light
```

- **auto** is the default and is what an absent stored key reads as, so a
  television that has never opened the menu keeps whatever the server's own
  clock decides — which is the behaviour every 0.4.13 pack already has.
- The header rides the poll rather than the URL on purpose. **The URL is what
  the renderer re-arms on**, so putting the preference in a query parameter
  would make every preference change look like a move to a different room.
- **A server that ignores the header is correct**, not broken: it keeps
  deciding on its own, which is exactly `auto`. That is what makes this
  additive for any server older than the app.
- Nothing is applied locally. The renderer does not recolour anything itself —
  it asks, and the poll brings back a scene painted the way it asked.
- **Changing the row polls immediately** (0.4.15). The viewer is standing in
  front of the television having just pressed the button, and a repaint that
  waited out `fetch.ms` read as a press that had not taken, so the press issues
  one extra poll of its own and the repaint costs a round trip instead of up to
  a full interval. The repeating timer is NOT re-armed — re-arming it would
  slide every later poll to a phase set by when somebody opened a menu, and the
  cadence stays the server's to set. If a poll is already in flight it carries
  the old value and cannot be recalled, so that press lands on the next
  ordinary tick instead; a server therefore still sees nothing it has not
  always seen — only, sometimes, one poll sooner.
...
**The mutes are not part of this contract.** They are a property of one
television in one room: the renderer stores them in `NSUserDefaults` and the
...
**The Screen row is stored the same way and for the same reason** — one
television, one room — but unlike the mutes it has to be TOLD to somebody,
because the server is what paints the scene. Sending it per request rather than
storing it server-side is what keeps that true without inventing the
per-display storage that does not exist: the preference lives on the device,
...
Every field here is additive: a scene that carries none of them renders exactly as it did in 0.4.14. `done.md` says why each exists.

### Document-level

| field | shape | behaviour |
|---|---|---|
| `open_url` | string | Handed to `UIApplication openURL` (the app that registered the scheme) once per distinct value; absent forgets the last value. The app must declare the scheme with `--query-schemes` only if it also wants it probed; opening needs no declaration. |
| `audio` | `{"url": "...", "volume": 0.35, "loop": true}` or `{"stop": true}` | A stream under the scene, no layer. Same url keeps playing; a new url replaces it; `stop` or an empty url stops it; **absent leaves it playing**. Volume follows the soundtrack mute. |
| `keyboard_submit` | path template containing `{q}` | Presents the system search keyboard (dictation included) once per template. Every change fetches the template (debounced 0.35 s, newest query wins) and shows that scene's `grid` as focusable tiles under the keyboard; a tile's `select` path opens on Select. **The `{q}` rule:** letters and digits kept, spaces, `_` and `-` become `_`, everything else dropped, and the empty query is `-`. The path is resolved against the origin being polled. |
| `keyboard_placeholder` | string | The search field's placeholder. |
| `compact` | `true` | Tighter column spacing and margins for a dense scene. |
| `forget_device` | `true` | Drops the per-installation device id and rebuilds from the boot document with a new one (sign out as a new television). Honoured once, not per poll. |

A distribution pack that must not carry a device id bakes `__HEY_TV_DEVICE__` wherever the id appears; the renderer substitutes the id it minted for this installation (`NSUserDefaults` `hey_ios_tv.device_id`) before reading any url.

### Widgets and slots

```json
{"kind": "grid", "id": "items", "columns": 4, "selected": 2, "focused": true,
 "tile": {"width": 380, "height": 300}, "clip_height": 0,
 "items": [{"id": "...", "image": "https://...", "symbol": "play.rectangle", "title": "...",
            "meta": "2025 · YouTube", "stars": 3, "badge": "YouTube", "select": "/path"}]}
{"kind": "chips", "id": "sections", "selected": 1, "focused": false, "items": ["All", "Video", "TV"]}
{"kind": "spinner", "tint": "#8FA3B8", "size": 16}
{"kind": "listrow", "text": "YouTube", "on": true, "focused": false, "width": 1100}
{"kind": "text", "text": "...", "slot": "rail", "max_width": 1200, "min_width": 600, "line_spacing": 12, "paragraph_spacing": 18}
```

- `grid`: fixed-size tiles, 16:9 artwork of the tile width, the `symbol` until the image loads, `stars` < 0 draws no stars. The server owns the cursor (`selected`); with `focused` the selected tile lifts and the rest dim. `clip_height` shows only the top band of full-size tiles.
- `chips`: words with a bar under `selected`; a full accent bar when `focused`, a dimmed one otherwise.
- `spinner`: three dots animating in sequence (`tint`, dot `size` in points).
- `listrow`: a settings row with an on/off symbol and a focus band.
- `slot: "rail"`: any widget may sit in a left column beside the content; without a rail widget the layout is unchanged.
- Text widgets take `max_width` (default 1760), `min_width`, `line_spacing` and `paragraph_spacing` in points.

### Request header

Every scene poll carries `X-Hey-TV-Apps`: the declared `LSApplicationQueriesSchemes` (the tool's `--query-schemes`) that `canOpenURL` answers yes for, comma-joined, probed once per launch. Empty means none answered or none were declared.

### Presses

A remote press keeps the scene on screen, sends the fetch its interim document names at once, and cancels an older fetch still in flight. The renderer logs `press -> scene on screen in N ms`. Returning to the app from another app polls immediately.
