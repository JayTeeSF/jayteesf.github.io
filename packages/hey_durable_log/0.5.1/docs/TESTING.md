...
THE TOOLCHAIN IMAGE IS CHECKED, NOT ASSUMED. `bin/check-cluster`,
`bin/check-partition` and `bin/check-pooling` all resolve their image through
`bin/require-toolchain-image` first, which refuses one too old for the harness
before any container starts and names what is missing from its `/src/stdlib`.
The list it checks is read out of the harness sources, so a new `stdlib:` import
is covered the day it is added rather than the day someone remembers.

That preflight exists because the default went stale and nothing said so. Every
green partition run on record had passed `HEY_TOOLCHAIN` by hand, so nobody had
exercised the tag the repository itself pins, and it had rotted twice over: the
image was Alpine while `docker/Dockerfile.partition` installed iptables with
`apt-get` (a bare `exit code: 127`), and underneath that it predated
`stdlib:Actor`, which surfaced as `follower b never reported that it was
listening` — three containers in, reading exactly like a replication bug.
Build a current image with `bin/build-toolchain-image`; it prints the tag.
