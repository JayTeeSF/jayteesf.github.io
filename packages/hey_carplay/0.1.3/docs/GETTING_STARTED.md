...
In an app, pin `hey_ios` 0.3.1 and `hey_carplay` 0.1.3 in `hey-package.json`, run `hey package ensure --project . --update-lock`, and pass the installed root to the build tool:

```sh
.hey/packages/hey_ios/0.3.1/tools/hey-ios-api-app.sh ... --extension .hey/packages/hey_carplay/0.1.3
```
