...
Install or unzip the package source under `$HOME/dev/hey_record`, then configure Hey and the shared packager:

```sh
export HEY_ROOT="$HOME/dev/hey-lang-bootstrap-plan"
export HEY_PACKAGER_ROOT="$HOME/dev/hey_packager"
cd "$HOME/dev/hey_record"
bin/check
```
...
bin/check
bin/release
unzip -tq "dist/hey_record-0.2.2.zip"
bin/source-zip
