...
### When the code that needs the journal cannot be handed it

A consumer normally keeps the handle `DurableLog.open` returns for as long as its
process runs — that is what the exclusive writer lock is for, and opening per
operation costs a full recovery scan each time.

Some callers cannot hold anything. A Distributed Hey call handler is a plain
function of the request body, and Hey has no top-level mutable state and no
closures, so a handler cannot capture what `program` opened. For those,
`open_retained` hands back the journal this process already holds:

```hey
# in `program`, once, and never closed
let held = DurableLog.open_retained(directory, bounds)

# in a request handler, as often as you like
let journal = DurableLog.open_retained(directory, bounds).value
# ... use it ...
let closed = DurableLog.close(journal)
```

The first call opens and scans; every later one takes a reference on the same
journal. `close` drops one reference, and the journal closes — releasing the
writer lock — only when the last one goes. So `program` holding one reference is
what keeps the count off zero.

Three things to know before using it:

- **It is separate from `open` on purpose.** A shared handle carries the queue
  capacity, batch size and window that its *first* caller chose. Asking for the
  same journal under different bounds is refused by name
  (`durable_log_open_retained_conflict`) rather than answered with someone
  else's broker.
- **`open` is unchanged.** A second plain `open` on a journal this process holds
  is still refused `durable_log_open_locked`.
- **Retention is measurable, not a claim.** `DurableLog.retained_opens(journal)`
  reports how many times this process really opened — and so recovery-scanned —
  that journal. It is what `bin/check-cluster` asserts against, because every
  correctness assertion passes whether the journal was opened once or once per
  request.

Journals are matched by device and inode rather than by the path you typed, so
two spellings of one directory, a symlink, or the same volume mounted twice all
resolve to one entry instead of deadlocking against the writer lock.
...
says load.window_waits     # batches that waited out the batch window
...
`window_waits` is the other side of that. The writer holds the first record of a
batch for `batch_window_microseconds` so concurrent callers can join it, and with
one caller in flight that wait is for company that provably is not coming -- a
caller blocked in `append` cannot append again. So a solitary writer skips the
window entirely and `window_waits` stays at 0; under concurrency it rises, which
is the window doing the job it exists for. It is counted rather than timed so
that "we stopped waiting when waiting could not help" is something you can check
instead of believe.
