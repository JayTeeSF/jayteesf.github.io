...
- **Card-table widget family** (quality bar set by the maintainer's
  solitaire web app, 2026-07-23 — felt table, shadowed cards, fanned
  stacks; the renderer must reach at least that polish):
  - `{"kind": "card", "rank": "K", "suit": "spades", "face": "up"}`
    — renderer draws a rounded-rect card at 2.5:3.5 proportions,
    white face with corner rank+suit indices and a large center pip
    (♠♥♦♣; hearts/diamonds red, spades/clubs near-black), soft drop
    shadow; `"face": "down"` draws the patterned card back.
  - `{"kind": "stack", "cards": [...], "fan": "down|right|none",
     "overlap": 0.22}` — fanned piles (tableau-style) or squared
    stacks; empty stacks render a dashed outline slot.
  - `{"kind": "hand", "cards": [...], "label": "Dealer: 17"}` —
    a horizontal card row with a caption, the blackjack primitive.
  - Recommended scene pairing: `"background": {"felt": "#1E4D3B"}`
    renders the table gradient rather than a flat color.
  Card GEOMETRY and STYLE are renderer-owned (one polished
  implementation serves every card game); apps supply only rank,
  suit, facing, and arrangement.
