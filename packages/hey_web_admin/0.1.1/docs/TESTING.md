...
`specs/generic_cagents_spec.hey` points the admin at `generic_cagents`,
...
admin produced. It finishes by asserting the original record is
unmodified.

generic_cagents has since retired that web app (archived in 1132fd0,
deleted in 5d1adcc) and was never a published package. So the app's
`domain/project.hey` and its record `db/projects/projects/p1.json` are
kept **verbatim** under `specs/fixtures/generic_cagents/`, copied from
generic_cagents commit cd715dd, with their sha256 recorded in the spec
header. They are the app's files, not fixtures written to fit the admin,
and they are read-only. The spec used to import
`/Users/jthomas/dev/generic_cagents/domain/project.hey` by absolute path
and went red when that file moved.
